...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : lun. 02 mai 2022 à 15:25
...
CREATE TABLE IF NOT EXISTS `dresseur` (
  ...
  `pieces` int NOT NULL DEFAULT '5000',
  PRIMARY KEY (`id`)
...
DROP TABLE IF EXISTS `dresseur_pokemon`;
CREATE TABLE IF NOT EXISTS `dresseur_pokemon` (
...
CREATE TABLE IF NOT EXISTS `ref_pokemon` (
  ...
  `type_2` int NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=152 DEFAULT CHARSET=utf8mb3 ROW_FORMAT=COMPACT;
...
ALTER TABLE `dresseur_pokemon`
  ADD CONSTRAINT `dresseur_pokemon_ibfk_1` FOREIGN KEY (`id_dresseur`) REFERENCES `dresseur` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  ADD CONSTRAINT `dresseur_pokemon_ibfk_2` FOREIGN KEY (`id_pokemon`) REFERENCES `pokemon` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;

--
-- Contraintes pour la table `pokemon`
--
ALTER TABLE `pokemon`
  ADD CONSTRAINT `pokemon_ibfk_1` FOREIGN KEY (`espece`) REFERENCES `ref_pokemon` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  ADD CONSTRAINT `pokemon_ibfk_2` FOREIGN KEY (`id_dresseur`) REFERENCES `dresseur` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
COMMIT;
